...
public class ScrewDriverTable : ToolTable
{
	...
	protected override bool checkCondition(){
		if (ItemSlot.transform.GetChild(0)){
			...
			if (item.CompareTag("A1") && item.GetComponent<SphereProjector>().spoiled == false || item.CompareTag("B2") && item.GetComponent<Propeller>().spoiled == false || item.CompareTag("C2") && item.GetComponent<Engine>().spoiled == false){
				return true;
			}else{
				return false;
			}
