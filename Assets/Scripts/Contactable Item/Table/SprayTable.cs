...
public class SprayTable : ToolTable
{
	...
	protected override bool checkCondition(){
		if (ItemSlot.transform.GetChild(0)){
			...
			if (item.CompareTag("Crate") && item.GetComponent<Crate>().dirty == true){
				return true;
			}else{
				return false;
			}
	...
	public override void makeProduct(){
		if (checkCondition()){
			...
			if(makeProgress >= 10f){
				...
				itemPart.transform.GetChild(1).gameObject.SetActive(false);
				itemPart.transform.GetChild(0).gameObject.SetActive(true);
			}
		}else{
